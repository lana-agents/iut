...
* [`LocalConstruct/`](Iut/Concrete/LocalConstruct) — the **construction** of
  ...
  the `p`-adic logarithm and log-shells of a local field (`PadicLog.lean`), the log-shell
  of a packet — the `(R_I)^∼`-module generated by their tensor product, containing
  `(R_I)^∼` — with the automorphism invariance of IUT IV Proposition 1.2 (`LogShell.lean`),
...
| `EllipticCurveData.CurveArithmetic` | Prop 1.8 | six of ten fields **proved** (`CurveArithmetic.ofCore`); for the tripod, `√−1 ∈ F`, `E[6]` rational and `F/F_mod` Galois of degree prime to `ℓ` are **proved** (`Iut/Tripod/Galois.lean`); stable reduction remains a `Prop` |
...
* [`CurveFacts.lean`](Iut/Tripod/CurveFacts.lean),
  ...
  and stable reduction of `E_λ/F_λ`; that `F_λ/ℚ(j)` is Galois of degree prime to `ℓ ≥ 7`
  is proved in [`Galois.lean`](Iut/Tripod/Galois.lean)); the remaining
...
`∀ K d, ∃ T_K, CurveFactsProp … K d T_K`, the tower arithmetic
