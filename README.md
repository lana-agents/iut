...
* [`LocalConstruct/`](Iut/Concrete/LocalConstruct) — the **construction** of
  ...
  (`ResidueField.lean`, `Hull.lean`). The residual input is the one field of
  `Iut.LocalConstruct.LocalTheoryFacts K`: Proposition 1.4(iii).
...
| --- | --- | --- |
| `Iut.LocalTheory K` | tensor packets, log-shells, Haar log-volume, hulls, Props 1.4/1.5 | **constructed** (`Iut.LocalConstruct.concreteLocalTheory`); residual `Prop` `LocalTheoryFacts K` (one field, Prop. 1.4(iii)), padic-log-volume [#1462](https://taxis.lana.merten.dev/issues/1462) |
