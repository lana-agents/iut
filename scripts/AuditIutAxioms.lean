...
/-!
Axiom audit for the `Iut` library (the Corollary 3.12 variant strand, taxis #33):
every public declaration must depend only on the standard axioms. Mirrors
`scripts/AuditAxioms.lean`, which audits `Iut4Sec1`.
-/

open Lean Elab Command

private def auditedRoot : Name := `Iut
...
private def writeSelectedNames (names : Array Name) : CommandElabM Unit := do
  let some output ← IO.getEnv "IUT_AUDIT_OUTPUT" |
    throwError "IUT_AUDIT_OUTPUT is required in manifest mode"
  IO.FS.writeFile output (renderNames names)

run_cmd do
  let mode ← IO.getEnv "IUT_AUDIT_MODE"
  match mode with
  | some "manifest" =>
  ...
  | _ => liftCoreM do
      ...
      if declarations.isEmpty then
        throwError "all-public audit selected no Iut declarations"
      ...
      logInfo m!"audited {declarations.size} public Iut declaration(s)"
