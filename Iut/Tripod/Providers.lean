...
propositions (`Iut.Tripod.CurveProps`): the `n`-torsion of `E_λ(ℚ̄)` is a rank-two
`ℤ/n`-module, and `F_λ/F_mod` is Galois of degree prime to `ℓ` for `ℓ ≥ 7`. The stable
reduction of `E_λ/F_λ` at every finite place is **proved** (`Iut.Tripod.stable_reduction`):
from the Legendre model at the places of odd residue characteristic, and from the rational
`3`-torsion (which follows from `E_λ[3](ℚ̄) ≅ (ℤ/3)²`) at the places over `2`.
-/
