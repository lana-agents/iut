...
* for an extension `K/k` of number fields: the places of `K` over a place `u` of `k` are
  ...
  `∑_{v ∣ u} e(v/u) f(v/u) = [K : k]` (`Iut.sum_relLocalDeg_liesOver_tower`), and the bounds
...
theorem sum_relLocalDeg_liesOver_tower (u : FinitePlace k) (s : Finset (FinitePlace K))
...
theorem sum_placesOver_relLocalDeg (u : FinitePlace k) :
  ...
  sum_relLocalDeg_liesOver_tower u _ (mem_placesOver u)
