...
/-!
# The log-shell of a tensor packet at a prime (taxis #4, #278)

The **log-shell** `𝓘_I` of a nonarchimedean packet (`LocalTheory.logShell` at a prime) is the
`(R_I)^∼`-module generated by the tensor product `⊗_j 𝓘_{c j}` of the log-shells
`𝓘_v = (2p)⁻¹·log(𝓞_v^×)` of the factors (`Iut.logShell`, `PadicLog.lean`), transported to
the factors `Factor K p v` (`factorLogShell`): the `ℤ_p`-span of the products `a·(⊗_j y_j)`
with `a ∈ (R_I)^∼` and `y_j ∈ 𝓘_{c j}` (`logShellSet`, `logShell`, `mul_mem_logShell`). The
tensor product `⊗_j 𝓘_{c j}` itself is `tensorLogShell` (IUT III, Proposition 3.2). We
prove:

* `order_subset_tensorLogShell`, `integral_subset_logShell`: `R_I ⊆ ⊗_j 𝓘_{c j}` (factorwise
  `closedBall_subset_logShell`, IUT III, Proposition 1.2) and `(R_I)^∼ ⊆ 𝓘_I`;
* `isOpen_logShell`, `isClosed_logShell`, `isCompact_logShell`: `𝓘_I` is a compact open
  subgroup (it contains `R_I`, and is bounded since `(R_I)^∼` is compact and the factor
  log-shells are bounded);
* `logShell_eq_integral_of_unramified`: at an odd prime unramified in every factor,
  `⊗_j 𝓘_{c j} = R_I` (`tensorLogShell_eq_order_of_unramified`, factorwise
  `logShell_eq_closedBall`) and `𝓘_I = (R_I)^∼`;
* `mapAlgHom_image_logShell_subset`: the indeterminacy automorphisms preserve `𝓘_I`
  ...
  convergent power series), so it preserves `𝓘_v` (`RingHom.image_logShell_subset`); and
  they preserve `(R_I)^∼` (`mapAlgHom_image_integral_subset`);
* `thetaShell_admissible` at a prime.

## Why the `(R_I)^∼`-module generated by `⊗_j 𝓘_{c j}`

The tensor product `⊗_j 𝓘_{c j}` alone does not contain the maximal order `(R_I)^∼`, as the
interface (`LocalTheory.integral_subset_logShell`, from `Iut/Cor312/Container.lean`)
requires: for `p = 3` and `K_w = ℚ_3(ζ_3)` the log-shell is `𝓘_w = 𝓞_w` (the principal units
are `μ_3·(1 + 𝔪²)` and `log(1 + 𝔪²) = 𝔪² = 3𝓞_w`), so for the packet `K_w ⊗ K_w ≅ K_w × K_w`
the tensor product of the log-shells is `𝓞_w ⊗_{ℤ_3} 𝓞_w = R_I`, which has index `3` in
`(R_I)^∼ = 𝓞_w × 𝓞_w`. The `(R_I)^∼`-module generated by `⊗_j 𝓘_{c j}` is the least hull
region containing the tensor product of the log-shells; it has all the properties of the
log-shell used by the container: it contains `(R_I)^∼`, is compact and open, equals `(R_I)^∼`
at odd primes unramified in every factor, and is preserved by the indeterminacies.
...
/-- **The tensor product of the log-shells** `⊗_j 𝓘_{c j}` (IUT III, Proposition 3.2): the
`ℤ_p`-span of the elementary tensors of tuples of elements of the factor log-shells. -/
noncomputable def tensorLogShell : Set (Tensor K (.finite p) c) :=
  Submodule.span ℤ_[p] (logShellSet p c)

/-- **The log-shell of a packet** `𝓘_I` as a `ℤ_p`-submodule: the `(R_I)^∼`-module generated
by the tensor product of the log-shells `⊗_j 𝓘_{c j}`, i.e. the `ℤ_p`-span of the products
`a·(⊗_j y_j)` with `a ∈ (R_I)^∼` and `y_j ∈ 𝓘_{c j}` (`mul_mem_logShell`). -/
noncomputable def logShellSubmodule : Submodule ℤ_[p] (Tensor K (.finite p) c) :=
  Submodule.span ℤ_[p] (integral p c * logShellSet p c)

/-- **The log-shell of a packet** `𝓘_I` (`LocalTheory.logShell` at a prime): the
`(R_I)^∼`-module generated by the tensor product of the log-shells `⊗_j 𝓘_{c j}`. -/
...
/-- `1 = ⊗_j 1` is an elementary tensor of elements of the factor log-shells. -/
lemma one_mem_logShellSet : (1 : Tensor K (.finite p) c) ∈ logShellSet p c := by
  refine ⟨fun j => ⟨1, ?_⟩, ?_⟩
  · rw [← map_one (factorMk p (c j))]
    exact factorMk_mem_factorLogShell_of_norm_le_one p (c j) 1 (by simp)
  · rw [PiTensorProduct.one_def]
    rfl

lemma logShellSet_subset_logShell : logShellSet p c ⊆ logShell p c := fun x hx =>
  Submodule.subset_span ⟨1, one_mem_integral p c, x, hx, one_mul x⟩

/-- `⊗_j 𝓘_{c j} ⊆ 𝓘_I`. -/
lemma tensorLogShell_subset_logShell : tensorLogShell p c ⊆ logShell p c :=
  Submodule.span_le.mpr (logShellSet_subset_logShell p c)

/-- The elementary tensors of integral tuples are elementary tensors of elements of the
factor log-shells. -/
lemma integralSet_subset_logShellSet : integralSet p c ⊆ logShellSet p c := by
  rintro _ ⟨a, rfl⟩
  exact ⟨fun j => ⟨factorMk p (c j) (a j).1,
    factorMk_mem_factorLogShell_of_norm_le_one p (c j) (a j).1 (a j).2⟩, rfl⟩

/-- `R_I ⊆ ⊗_j 𝓘_{c j}` (IUT IV, Proposition 1.2). -/
theorem order_subset_tensorLogShell : order p c ⊆ tensorLogShell p c :=
  Submodule.span_mono (integralSet_subset_logShellSet p c)

/-- `R_I ⊆ 𝓘_I`. -/
theorem order_subset_logShell : order p c ⊆ logShell p c :=
  (order_subset_tensorLogShell p c).trans (tensorLogShell_subset_logShell p c)

/-- **`(R_I)^∼ ⊆ 𝓘_I`** (IUT III, Proposition 1.2; `LocalTheory.integral_subset_logShell` at
a prime). -/
theorem integral_subset_logShell : integral p c ⊆ logShell p c := fun a ha =>
  Submodule.subset_span ⟨a, ha, 1, one_mem_logShellSet p c, mul_one a⟩

/-- **`𝓘_I` is an `(R_I)^∼`-module.** -/
theorem mul_mem_logShell {a x : Tensor K (.finite p) c} (ha : a ∈ integral p c)
    (hx : x ∈ logShell p c) : a * x ∈ logShell p c := by
  rw [mem_logShell] at hx ⊢
  induction hx using Submodule.span_induction with
  | mem x hx =>
    obtain ⟨b, hb, y, hy, rfl⟩ := hx
    rw [← mul_assoc]
    exact Submodule.subset_span ⟨a * b, mul_mem_integral p c ha hb, y, hy, rfl⟩
...
/-- The products of elements of `(R_I)^∼` with elementary tensors of elements of the factor
log-shells are bounded: `(R_I)^∼` is compact and the elementary tensors are bounded. -/
lemma exists_norm_mul_logShellSet_le :
    ∃ M : ℝ, 0 ≤ M ∧ ∀ x ∈ integral p c * logShellSet p c, ‖x‖ ≤ M := by
  obtain ⟨M₀, -, hM₀⟩ := exists_norm_logShellSet_le p c
  have hS : Bornology.IsBounded (logShellSet p c) := isBounded_iff_forall_norm_le.mpr ⟨M₀, hM₀⟩
  have hc : IsCompact (integral p c * closure (logShellSet p c)) :=
    (isCompact_integral p c).mul hS.isCompact_closure
  obtain ⟨M, hM⟩ := isBounded_iff_forall_norm_le.mp hc.isBounded
  exact ⟨max M 0, le_max_right _ _, fun x hx =>
    (hM x (Set.mul_subset_mul_left subset_closure hx)).trans (le_max_left _ _)⟩

/-- **`𝓘_I` is bounded.** -/
lemma exists_logShell_subset_closedBall : ∃ M : ℝ, logShell p c ⊆ Metric.closedBall 0 M := by
  obtain ⟨M, hM0, hM⟩ := exists_norm_mul_logShellSet_le p c
...
/-- At an odd prime unramified in every factor, the elementary tensors of elements of the
factor log-shells lie in `R_I` (factorwise `factorLogShell_eq_of_unramified`). -/
theorem logShellSet_subset_order_of_unramified (hodd : Odd (p : ℕ))
    (hunr : ∀ j w, c j = Place.finite w → ramIdx K w = 1) : logShellSet p c ⊆ order p c := by
  rintro _ ⟨a, rfl⟩
...
/-- **At an odd prime unramified in every factor, `⊗_j 𝓘_{c j} = R_I`** (IUT I,
Definition 5.4.5). -/
theorem tensorLogShell_eq_order_of_unramified (hodd : Odd (p : ℕ))
    (hunr : ∀ j w, c j = Place.finite w → ramIdx K w = 1) : tensorLogShell p c = order p c :=
  Set.Subset.antisymm
    (Submodule.span_le.mpr (logShellSet_subset_order_of_unramified p c hodd hunr))
    (order_subset_tensorLogShell p c)

/-- **At an odd prime unramified in every factor, `𝓘_I = (R_I)^∼`** (IUT I, Definition 5.4.5;
`LocalTheory.logShell_eq_integral` at a prime). -/
theorem logShell_eq_integral_of_unramified (hodd : Odd (p : ℕ))
    (hunr : ∀ j w, c j = Place.finite w → ramIdx K w = 1) : logShell p c = integral p c := by
  refine Set.Subset.antisymm ?_ (integral_subset_logShell p c)
  change (logShellSubmodule p c : Set (Tensor K (.finite p) c)) ⊆
    (Subalgebra.toSubmodule (integralClosure ℤ_[p] (Tensor K (.finite p) c)) :
      Set (Tensor K (.finite p) c))
  refine Submodule.span_le.mpr ?_
  rintro _ ⟨a, ha, x, hx, rfl⟩
  exact mul_mem_integral p c ha
    (order_subset_integral p c (logShellSet_subset_order_of_unramified p c hodd hunr hx))

/-- **The indeterminacy automorphisms preserve `𝓘_I`** (IUT IV, Proposition 1.2;
`LocalTheory.indAut_logShell` at a prime): they preserve `(R_I)^∼` and the elementary
tensors of elements of the factor log-shells. -/
theorem mapAlgHom_image_logShell_subset
    (σ : ∀ j, Factor K p (c j) ≃ₐ[ℚ_[p]] Factor K p (c j)) :
    mapAlgHom (.finite p) c σ '' logShell p c ⊆ logShell p c := by
  rintro _ ⟨x, hx, rfl⟩
  rw [mem_logShell] at hx ⊢
  induction hx using Submodule.span_induction with
  | mem x hx =>
    obtain ⟨a, ha, _, ⟨b, rfl⟩, rfl⟩ := hx
    rw [map_mul, mapAlgHom_tprod]
    refine Submodule.subset_span ⟨_, mapAlgHom_image_integral_subset p c σ ⟨a, ha, rfl⟩, _,
      ⟨fun j => ⟨σ j (b j), algEquiv_image_factorLogShell_subset p (c j) (σ j)
        ⟨_, (b j).2, rfl⟩⟩, rfl⟩, rfl⟩
